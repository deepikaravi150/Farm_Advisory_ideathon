...
export default function SoilReportUpload({ onUploadSuccess }: Props) {
  ...
  const onDrop = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      if (!res.ok) { setError(typeof data.error === 'string' ? data.error : t('uploadFailed')); return; }
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-semibold text-green-800">{t('extracted')}</h4>
            {result.labName && <span className="text-sm text-green-600 ml-auto">{result.labName}</span>}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {[
              { label: t('ph'), value: result.ph },
              { label: t('nitrogen'), value: result.nitrogen },
              { label: t('phosphorus'), value: result.phosphorus },
              { label: t('potassium'), value: result.potassium },
            ].map(item => (
