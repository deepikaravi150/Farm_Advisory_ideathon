...
export default function LandMapWidget({ coordinates, landArea }: LandMapWidgetProps) {
  if (!coordinates?.length) return (
    <div className="bg-white rounded-2xl p-5 shadow border border-gray-100 flex flex-col items-center justify-center h-48 text-gray-400">
      ...
      <p className="text-sm">No land coordinates recorded</p>
    </div>
  );
    ...
    <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-100">
      <div className="flex items-center justify-between p-3 border-b border-gray-100">
        <span className="flex items-center gap-2 font-medium text-gray-700">
          <MapPin className="w-4 h-4 text-brand-600" /> Your Land
        </span>
        <span className="text-sm text-gray-500">{landArea} acres</span>
      </div>
      <div className="h-48">
        <MapContainer
          center={center}
