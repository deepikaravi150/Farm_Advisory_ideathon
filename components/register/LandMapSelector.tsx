...
interface Props { onChange: (coords: Coordinate[]) => void; }
...
export default function LandMapSelector({ onChange }: Props) {
  ...
  async function searchLocation(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery + ' Tamil Nadu India')}&limit=1`,
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-600 flex items-center gap-1">
          ...
          Mark your land boundary on the map
        </p>
        {points.length > 0 && (
      ...
      <div className="flex gap-2 items-center">
        {!drawing && !finished && (
          <button onClick={startDrawing}
            ...
            <Crosshair className="w-4 h-4" /> Draw Land Boundary
          </button>
        )}
        ...
        {drawing && (
          <>
            <p className="flex-1 text-sm text-brand-700 bg-brand-50 border border-brand-200 rounded-xl px-3 py-2 flex items-center gap-2">
              ...
              Click to add points · Double-click or press Done to finish
            </p>
            {points.length >= 3 && (
        ...
        {finished && (
          <div className="flex-1 flex items-center gap-2 text-sm text-green-700 bg-green-50 border border-green-200 rounded-xl px-3 py-2">
            ...
            {points.length} boundary points saved
            <button onClick={startDrawing} className="ml-auto text-xs text-green-600 underline">Redraw</button>
          </div>
        )}
      </div>
      ...
      {!drawing && !finished && (
        <p className="text-xs text-gray-400 text-center">
          First search your location above to zoom in, then click "Draw Land Boundary"
        </p>
      )}
    </div>
  );
