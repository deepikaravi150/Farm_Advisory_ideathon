...
export default function CropSuggestions({ crops, onSelect }: CropSuggestionsProps) {
  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-4">Recommended Crops for Your Land</h3>
          ...
          <div key={crop.cropName}
            ...
            <div className="space-y-1.5 mb-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">Revenue est.: <span className="font-medium text-earth-700">{crop.estimatedRevenue}</span></span>
              </div>
              <div className="flex items-center gap-2">
                <IndianRupee className="w-3.5 h-3.5 text-red-500" />
                <span className="text-gray-600">Budget: <span className="font-medium">₹{crop.totalBudgetEstimate?.toLocaleString('en-IN')}</span></span>
              </div>
            </div>
            <button onClick={() => onSelect(crop)}
              ...
              <CheckCircle2 className="w-4 h-4" /> Select This Crop
            </button>
          </div>
        ))}
      </div>
