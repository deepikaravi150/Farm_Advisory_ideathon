...
import { Calendar, IndianRupee, AlertTriangle, CheckCircle2, Circle } from 'lucide-react';
    ...
    <div className="relative">
      <div className="flex flex-wrap gap-4 mb-4 text-sm">
        ...
        <span className="bg-earth-100 text-earth-700 px-3 py-1.5 rounded-full flex items-center gap-1">
          <IndianRupee className="w-3.5 h-3.5" />{t('totalBudget')}: ₹{totalBudget.toLocaleString('en-IN')}
        </span>
        <span className="bg-sky-100 text-sky-700 px-3 py-1.5 rounded-full flex items-center gap-1">
          <Calendar className="w-3.5 h-3.5" />{t('harvest')}: {formatDate(harvestDate)}
        </span>
        <span className="bg-green-100 text-green-700 px-3 py-1.5 rounded-full">{t('sellWindow')}: {sellWindow}</span>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 overflow-hidden" style={{ height: 320 }}>
        <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange}
          ...
          <MiniMap
            pannable
            zoomable
            // Smaller, semi-transparent so it doesn't cover the milestone flow chart.
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-500 mb-4 flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5" />{formatDate(selected.date)} · {selected.durationDays} {t('days')}
            </p>
            <div className="space-y-3 text-sm">
              <div><p className="font-medium text-gray-600 mb-1">{t('tasks')}</p><p className="text-gray-700 whitespace-pre-wrap">{selected.tasks}</p></div>
              {selected.estimatedCost > 0 && (
                <div className="flex items-center gap-2">
                  <IndianRupee className="w-4 h-4 text-earth-600" />
                  <span className="text-earth-700 font-medium">{t('estimatedCost')}: ₹{selected.estimatedCost.toLocaleString('en-IN')}</span>
                </div>
              )}
            ...
            <button onClick={() => setSelected(null)} className="mt-4 w-full bg-brand-600 text-white py-2 rounded-xl hover:bg-brand-700">{t('close')}</button>
          </div>
        </div>
      )}
    </div>
