...
async function extractText(key: string, buf: Buffer): Promise<string> {
  ...
  if (lower.endsWith('.pdf')) {
    try {
      // Optional dependency — resolved from a variable specifier so the build
      // doesn't require it to be installed unless PDFs are actually ingested.
      const mod = 'pdf-parse';
      const pdfParse = (await import(mod)).default as (b: Buffer) => Promise<{ text: string }>;
      const data = await pdfParse(buf);
      return data.text;
    } catch {
      console.warn(`  ! Skipping PDF (install pdf-parse to ingest): ${key}`);
      return '';
    }
  }
